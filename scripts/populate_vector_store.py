...
from core.yelp.models import YelpBusinessRaw
...
def populate():
    ...
    chroma_store = ChromaStore(persist_directory="data/chroma")
    
    # --- NEW: Clear existing collection for a fresh demo state ---
    ...
    # Batch add to avoid potential overhead
    batch_size = 50
    ...
    # 6. Sample Retrieval
    queries = [
        "comfort food in Lagos",
        "expensive fine dining",
        "late night suya",
        "quiet place to work with coffee"
    ]
    
    for q in queries:
        logger.info(f"Testing retrieval for query: '{q}'...")
        query_emb = embedding_service.embed_text(q).tolist()
        results = chroma_store.query_businesses([query_emb], n_results=3)
        
        if results['ids'] and results['ids'][0]:
            for i in range(len(results['ids'][0])):
                logger.info(f"  Match {i+1}: {results['metadatas'][0][i]['name']} (Distance: {results['distances'][0][i]:.4f})")
        else:
            logger.warning(f"  Retrieval test for '{q}' returned no results!")
